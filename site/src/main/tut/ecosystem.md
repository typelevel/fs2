...
* The `cats-effect` library extends the typeclasses from `cats` with other more specifi typeclasses to describe effectful computations. These type-classes cover resource  bracketing, concurrency, asynchronicity, input-output, timing, and interruption. 
