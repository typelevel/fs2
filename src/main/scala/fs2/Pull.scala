...
/**
  ...
  */
final class Pull[+F[_], +O, +R] private (private val _fold: Fold[Nothing, Nothing, Nothing])
    extends AnyVal {

  private[fs2] def fold[F2[x] >: F[x], O2 >: O, R2 >: R]: Fold[F2, O2, R2] =
    _fold.asInstanceOf[Fold[F2, O2, R2]]
  ...
  def stream: Stream[F, O] =
    Stream.fromFold(this.fold[F, O, R].map(_ => ()))

  /** Applies the resource of this pull to `f` and returns the result. */
  def flatMap[F2[x] >: F[x], O2 >: O, R2](f: R => Pull[F2, O2, R2]): Pull[F2, O2, R2] =
    Pull.fromFold(fold[F2, O2, R].flatMap(r => f(r).fold))
...
object Pull {
  ...
  private[fs2] def fromFold[F[_], O, R](fold: Fold[F, O, R]): Pull[F, O, R] =
    new Pull(fold.asInstanceOf[Fold[Nothing, Nothing, Nothing]])
  ...
  val done: Pull[Nothing, Nothing, Unit] =
    fromFold[Nothing, Nothing, Unit](Fold.pure[Nothing, Nothing, Unit](()))
  ...
  def eval[F[_], R](fr: F[R]): Pull[F, Nothing, R] =
    fromFold(Fold.eval[F, Nothing, R](fr))
