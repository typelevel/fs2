...
import scalaz.stream2.wye.Request
import scalaz.{-\/, \/, \/-}
...
final case class Exchange[I, W](read: Process[Task, I], write: Sink[Task, W]) {
  ...
  def run(p:Process[Task,W] = halt, terminateOn:Request = Request.L):Process[Task,I] = {
    val y = terminateOn match {
      case Request.L => scalaz.stream2.wye.mergeHaltL[I]
      case Request.R => scalaz.stream2.wye.mergeHaltR[I]
      case Request.Both => scalaz.stream2.wye.mergeHaltBoth[I]
    }
