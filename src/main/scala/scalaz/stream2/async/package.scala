...
import scalaz.stream2.Process.emit
import scalaz.stream2.Process.halt
import scalaz.stream2.async.mutable.{BoundedQueue, WriterTopic, Topic, Signal}
import scalaz.stream2.merge.{JunctionStrategies, Junction}
...
package object async {
   ...
   * @return
   */
  private[stream2] def stateSignal[A](source: Process[Task, A])(implicit S:Strategy) : immutable.Signal[A] =
