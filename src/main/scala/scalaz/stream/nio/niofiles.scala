...
object niofiles {
  ...
  def linesR(src: => AsynchronousFileChannel)(implicit codec: Codec): Process[Task,String] =
    textR(src)(codec).pipe(text.lines)
   ...
   * is automatically closed on completion.
   */
  def textR(src: => AsynchronousFileChannel)(implicit codec: Codec): Process[Task,String] = {
    val f = decodeByteBuff(codec)
    Process.constant(1024).through(chunkReadBuffer(src)).flatMap { buff =>
      val charbuff = f(buff)
    ...
    resource(Task.delay(src))(
      ...
      Task.now { (size: Int) => Task.async[ByteBuffer] { cb =>
        ...
        src.read(buff, pos, null, new CompletionHandler[Integer, Null] {
          ...
          override def failed(exc: Throwable, attachment: Null): Unit = cb(-\/(exc))
        })
      }
      }
    }
