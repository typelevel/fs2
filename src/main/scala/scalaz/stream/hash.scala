...
trait hash {
  ...
  def md2: Process1[Bytes, Bytes] = messageDigest("MD2")

  /** Computes the MD5 hash of the input elements. */
  def md5: Process1[Bytes, Bytes] = messageDigest("MD5")

  /** Computes the SHA-1 hash of the input elements. */
  def sha1: Process1[Bytes, Bytes] = messageDigest("SHA-1")

  /** Computes the SHA-256 hash of the input elements. */
  def sha256: Process1[Bytes, Bytes] = messageDigest("SHA-256")

  /** Computes the SHA-384 hash of the input elements. */
  def sha384: Process1[Bytes, Bytes] = messageDigest("SHA-384")

  /** Computes the SHA-512 hash of the input elements. */
  def sha512: Process1[Bytes, Bytes] = messageDigest("SHA-512")
  ...
  private def messageDigest(algorithm: String): Process1[Bytes, Bytes] = {
    ...
    go(MessageDigest.getInstance(algorithm))
  }
}
