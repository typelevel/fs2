...
class ExchangeSpec extends Properties("Exchange") {
  ...
  property("loopBack") = protect {
    ...
    l.flatMap(_.run(emitAll(xs))).take(xs.size).runLog.unsafePerformSync.toSeq == xs.map(_.toString)
  }

  property("emitHead") = protect {
    val xs = 1 until 10
    val l = Exchange.loopBack[Int, Int](emitAll(xs) ++ process1.id)
    l.flatMap(_.run(emitAll(xs))).take(xs.size + xs.size / 2).runLog.unsafePerformSync.toSeq == xs ++ xs.take(xs.size / 2)
  }

  property("loopBack.terminate.process1") = protect {
    val xs = 1 until 10
    val l = Exchange.loopBack[Int, Int](process1.take(5))
    l.flatMap(_.run(emitAll(xs))).runLog.unsafePerformSync.toSeq == xs.take(5)
  }
  ...
  property("mapO") = protect {
    ...
    l.flatMap(_.run(emitAll(xs))).take(xs.size).runLog.unsafePerformSync == xs.map(_.toString)
  }

  property("mapW") = protect {
    val xs = 1 until 10
    val l = Exchange.loopBack[Int, Int](process1.id).map(_.mapW[String](_.toInt))
    l.flatMap(_.run(emitAll(xs.map(_.toString)))).take(xs.size).runLog.unsafePerformSync == xs
  }
  ...
  property("pipeBoth") = protect {
    ...
    l.flatMap(_.run(emitAll(xs.map(_.toString)))).take(xs.size).runLog.unsafePerformSync.toSeq == xs.map(_ * 10).map(_.toString)
  }
  ...
  property("through") = protect {
    ...
    l.flatMap(_.run(emitAll(xs))).take(xs.size * xs.size).runLog.unsafePerformSync == xs.map(i => xs.map(i2 => (i, i2))).flatten
  }

  property("run.terminate.on.read") = protect {
    val ex = Exchange[Int,Int](Process.range(1,10),Process.constant(i => Task.now(())))
    ex.run(time.sleep(1 minute)).runLog.unsafePerformTimed(3000).unsafePerformSync == (1 until 10).toVector
  }


  property("run.terminate.on.write") = protect {
    val ex = Exchange[Int,Int](time.sleep(1 minute),Process.constant(i => Task.now(())))
    ex.run(Process.range(1,10), Request.R).runLog.unsafePerformTimed(3000).unsafePerformSync == Vector()
  }
  ...
  property("run.terminate.on.read.or.write") = protect {
    ...
    ("left side terminated" |: exL.run(time.sleep(1 minute), Request.Both).runLog.unsafePerformTimed(3000).unsafePerformSync == (1 until 10).toVector) &&
      ("right side terminated" |: exR.run(Process.range(1,10), Request.Both).runLog.unsafePerformTimed(3000).unsafePerformSync == Vector())
  }
