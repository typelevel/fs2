...
object WyeSpec extends Properties("wye")  {
  ...
  property("merge.queue.both-cleanup") = secure {
    ...
    p.runLog.timed(3 seconds).runAsync(sync.put)
  ...
  property("merge-drain-halt") = secure {
    ...
    val pm1 = effect.wye(Process(1 seconds, 2 seconds).toSource)(wye.merge).take(2)
    val pm2 = Process(3 seconds, 4 seconds).toSource.wye(effect)(wye.merge).take(2)

    (pm1 ++ pm2).runLog.timed(3 seconds).run.size == 4
  }
