...
//         wrapBuffer
//           .perform((appBuffer, netBuffer) => sslEngine.wrap(appBuffer, netBuffer))
...
//                         if (handshaking)
//                           Applicative[F].pure(
//                             EncryptResult.Handshake(Chunk.empty, doWrap(handshaking))
//                           )
...
//                           } else
//                         log("doWrap handshake finished but more in buffer") >> doWrap(handshaking)
//                     }
...
//                           handshakeFinishedRef
//                             .update(old => old >> gate.complete(()))
//                             .as(
//                               EncryptResult.Handshake(
//                                 out,
//                                 gate.get >> log("resuming original wrap") >> wrapBuffer
//                                   .input(Chunk.empty) >> doWrap(handshaking)
//                               )
//                             )
//                         }
//                       }
...
//         unwrapBuffer
//           .perform(sslEngine.unwrap(_, _))
//           .flatTap(result => log(s"doUnwrap result: $result"))
//           .flatMap { result =>
//             result.getStatus match {
//               case SSLEngineResult.Status.OK =>
//                 result.getHandshakeStatus match {
...
//                         } else
//                         log("doUnwrap handshake finished but there's more to unwrap") >> doUnwrap
//                     }
//                   case SSLEngineResult.HandshakeStatus.NEED_TASK =>
