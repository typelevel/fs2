...
object ThisModuleShouldCompile {
  ...
  val a3a = Stream(1,2,3).covary[IO].pull.uncons1.stream
  val a3b = Stream.eval(IO.pure(1)).pull.uncons1.stream
  ...
  val f = (Stream(1,2,3).covary[IO]).pull.uncons1.flatMapOpt { case (hd,_) => Pull.output1(hd).as(None) }.stream
  val g = Stream(1,2,3).pull.uncons1.flatMapOpt { case (hd,_) => Pull.output1(hd).as(None) }.stream
  val h = Stream(1,2,3).pull.uncons1.flatMapOpt { case (hd,_) => Pull.eval(IO.pure(1)) >> Pull.output1(hd).as(None) }.stream

  import scala.concurrent.ExecutionContext.Implicits.global
  val i = Stream.eval(IO.pure(1)).pull.unconsAsync.stream
