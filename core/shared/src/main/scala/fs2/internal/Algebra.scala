...
private[fs2] object Algebra {
  ...
  final case class OpenScope[G[_]](interruptible: Option[Concurrent[G]]) extends AlgEffect[PureK, Token]
  ...
  def stepLeg[F[_], O](leg: Stream.StepLeg[F, O]): FreeC[F, Nothing, Option[Stream.StepLeg[F, O]]] =
    Step[O](leg.next, Some(leg.scopeId)).map {
      _.map { case (h, id, t) => new Stream.StepLeg[F, O](h, id, t.asInstanceOf[FreeC[F, O, Unit]]) }
    }
                ...
                case Some(toClose) =>
                  if (toClose.parent.isEmpty) {
                    ...
                    extendedTopLevelScope.traverse_(_.close(ExitCase.Completed).rethrow) *>
                      F.flatMap(toClose.openAncestor)(
  ...
  private def translate0[F[_], G[_], O](
    ...
    def translateStep[X](next: FreeC[F, X, Unit], isMainLevel: Boolean): FreeC[G, X, Unit] =
      next.viewL match {
        case pure: FreeC.Result.Pure[Unit] => pure

        case failed: FreeC.Result.Fail => failed

        case interrupted: FreeC.Result.Interrupted[_] => interrupted

        case view: ViewL.View[F, X, y, Unit] =>
          view.step match {
            case output: Output[X] =>
              Output[X](output.values).transformWith {
