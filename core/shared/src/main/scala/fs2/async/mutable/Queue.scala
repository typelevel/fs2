...
//   def dequeueBatch1(batchSize: Int): F[Segment[A,Unit]]
//
//   /** Like `dequeueBatch1` but provides a way to cancel the dequeue. */
//   def cancellableDequeueBatch1(batchSize: Int): F[(F[Segment[A,Unit]], F[Unit])]
//
...
//       def dequeueBatch1(batchSize: Int): F[Segment[B,Unit]] =
//         self.dequeueBatch1(batchSize).map(_.map(f))
//       def cancellableDequeueBatch1(batchSize: Int): F[(F[Segment[B,Unit]],F[Unit])] =
...
//       */
//     final case class State(queue: Vector[A], deq: Vector[concurrent.Ref[F,Segment[A,Unit]]])
//
...
//               c.previous.deq.head.setAsyncPure(Segment.singleton(a)).as(true)
//           }
//
...
//
//         def cancellableDequeueBatch1(batchSize: Int): F[(F[Segment[A,Unit]],F[Unit])] =
//           concurrent.ref[F,Segment[A,Unit]].flatMap { r =>
//             qref.modify { s =>
...
//                     if (batchSize == 1)
//                       Segment.singleton(c.previous.queue.head)
//                     else
//                       Segment.indexedSeq(c.previous.queue.take(batchSize))
//                 }
//                 else r.get
//               }
//               val cleanup =
...
//           cancellableDequeueBatch1(1).map { case (deq,cancel) => (deq.map(_.head),cancel) }
//         override def dequeueBatch1(batchSize: Int): F[Segment[A,Unit]] = cancellableDequeueBatch1(batchSize).flatMap { _._1 }
//         def cancellableDequeueBatch1(batchSize: Int): F[(F[Segment[A,Unit]],F[Unit])] =
...
//           enqueue1(a).as(true)
//         def dequeue1: F[A] = cancellableDequeue1.flatMap { _._1 }
//         def dequeueBatch1(batchSize: Int): F[Segment[A,Unit]] = cancellableDequeueBatch1(batchSize).flatMap { _._1 }
...
//         override def dequeueBatch1(batchSize: Int): F[Segment[A,Unit]] = q.dequeueBatch1(batchSize)
//         def cancellableDequeueBatch1(batchSize: Int): F[(F[Segment[A,Unit]],F[Unit])] =
...
//           permits.increment >> q.cancellableDequeue1
//         override def dequeueBatch1(batchSize: Int): F[Segment[Option[A],Unit]] =
//           q.dequeueBatch1(batchSize)
//         def cancellableDequeueBatch1(batchSize: Int): F[(F[Segment[Option[A],Unit]],F[Unit])] =
//           permits.increment >> q.cancellableDequeueBatch1(batchSize)
//         def size = q.size
//         def full: immutable.Signal[F, Boolean] = Signal.constant(true)
//         def available: immutable.Signal[F, Int] = Signal.constant(0)
