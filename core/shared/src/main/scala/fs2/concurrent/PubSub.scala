...
//   def apply[F[_]: ConcurrentThrow: Ref.Mk, I, O, QS, Selector](
...
//         strategy: PubSub.Strategy[I, O, QS, Selector]
//     )(implicit F: ConcurrentThrow[F], G: Functor[G], mkRef: Ref.MkIn[G, F]): G[PubSub[F, I, O, Selector]] = {
...
//         def clearPublisher(token: Token)(exitCase: Resource.ExitCase): F[Unit] =
//           exitCase match {
//             case Resource.ExitCase.Completed => Applicative[F].unit
...
//         def clearSubscriberOnCancel(token: Token)(exitCase: Resource.ExitCase): F[Unit] =
//           exitCase match {
//             case Resource.ExitCase.Completed                    => Applicative[F].unit
//             case Resource.ExitCase.Errored(_) | Resource.ExitCase.Canceled => clearSubscriber(token)
//           }
