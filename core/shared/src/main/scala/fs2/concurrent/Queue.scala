...
//         maxSize: Int
//     )(implicit F: ConcurrentThrow[F]): G[NoneTerminatedQueue[F, A]] =
//       forStrategyNoneTerminated(PubSub.Strategy.closeDrainFirst(Strategy.boundedFifo(maxSize)))

//     /** Creates a queue which stores the last `maxSize` enqueued elements and which never blocks on enqueue. */
//     def circularBuffer[F[_], A](maxSize: Int)(implicit F: ConcurrentThrow[F]): G[Queue[F, A]] =
...
//     def noneTerminated[F[_], A](implicit F: ConcurrentThrow[F]): G[NoneTerminatedQueue[F, A]] =
...
//     def synchronous[F[_], A](implicit F: ConcurrentThrow[F]): G[Queue[F, A]] =
...
//     private[fs2] def forStrategy[F[_]: ConcurrentThrow, S, A](
...
//     private[fs2] def forStrategyNoneTerminated[F[_]: ConcurrentThrow, S, A](
...
//   /** Creates a queue with no size bound. */
//   def unbounded[F[_], A](implicit F: ConcurrentThrow[F]): F[Queue[F, A]] =
//     in[F].unbounded

//   /** Creates an unbounded queue that distributed always at max `fairSize` elements to any subscriber. */
//   def fairUnbounded[F[_], A](fairSize: Int)(implicit F: ConcurrentThrow[F]): F[Queue[F, A]] =
//     in[F].fairUnbounded(fairSize)

//   /** Creates a queue with the specified size bound. */
//   def bounded[F[_], A](maxSize: Int)(implicit F: ConcurrentThrow[F]): F[Queue[F, A]] =
//     in[F].bounded(maxSize)

//   /** Creates a bounded queue terminated by enqueueing `None`. All elements before `None` are preserved. */
//   def boundedNoneTerminated[F[_], A](
//       maxSize: Int
//   )(implicit F: ConcurrentThrow[F]): F[NoneTerminatedQueue[F, A]] =
//     in[F].boundedNoneTerminated(maxSize)

//   /** Creates a queue which stores the last `maxSize` enqueued elements and which never blocks on enqueue. */
//   def circularBuffer[F[_], A](maxSize: Int)(implicit F: ConcurrentThrow[F]): F[Queue[F, A]] =
...
//   /** Created a bounded queue that distributed always at max `fairSize` elements to any subscriber. */
//   def fairBounded[F[_], A](maxSize: Int, fairSize: Int)(implicit F: ConcurrentThrow[F]): F[Queue[F, A]] =
//     in[F].fairBounded(maxSize, fairSize)

//   /** Created an unbounded queue terminated by enqueueing `None`. All elements before `None`. */
//   def noneTerminated[F[_], A](implicit F: ConcurrentThrow[F]): F[NoneTerminatedQueue[F, A]] =
//     in[F].noneTerminated

//   /** Creates a queue which allows at most a single element to be enqueued at any time. */
//   def synchronous[F[_], A](implicit F: ConcurrentThrow[F]): F[Queue[F, A]] =
//     in[F].synchronous

//   /** Like [[synchronous]], except that any enqueue of `None` will never block and cancels any dequeue operation. */
//   def synchronousNoneTerminated[F[_], A](implicit F: ConcurrentThrow[F]): F[NoneTerminatedQueue[F, A]] =
...
//     /** Creates a queue with no size bound. */
//     def unbounded[F[_], A](implicit F: ConcurrentThrow[F]): G[InspectableQueue[F, A]] =
//       forStrategy(Queue.Strategy.fifo[A])(_.headOption)(_.size)

//     /** Creates a queue with the specified size bound. */
//     def bounded[F[_], A](maxSize: Int)(implicit F: ConcurrentThrow[F]): G[InspectableQueue[F, A]] =
//       forStrategy(Queue.Strategy.boundedFifo[A](maxSize))(_.headOption)(_.size)

//     /** Creates a queue which stores the last `maxSize` enqueued elements and which never blocks on enqueue. */
//     def circularBuffer[F[_], A](
//         maxSize: Int
//     )(implicit F: ConcurrentThrow[F]): G[InspectableQueue[F, A]] =
...
//   /** Creates a queue with no size bound. */
//   def unbounded[F[_], A](implicit F: ConcurrentThrow[F]): F[InspectableQueue[F, A]] =
//     in[F].unbounded

//   /** Creates a queue with the specified size bound. */
//   def bounded[F[_], A](maxSize: Int)(implicit F: ConcurrentThrow[F]): F[InspectableQueue[F, A]] =
//     in[F].bounded(maxSize)

//   /** Creates a queue which stores the last `maxSize` enqueued elements and which never blocks on enqueue. */
//   def circularBuffer[F[_], A](maxSize: Int)(implicit F: ConcurrentThrow[F]): F[InspectableQueue[F, A]] =
//     in[F].circularBuffer(maxSize)
// }
