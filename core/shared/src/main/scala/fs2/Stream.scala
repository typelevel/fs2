            ...
            .flatMap { running => // starts with 1 because outer stream is running by default
              Queue
                ...
                .map { outputQ => // sync queue assures we won't overload heap when resulting stream is not able to catchup with inner streams
                  ...
                  // if fails will end in queue failure
                          ...
                          available.acquire >>
                            ...
                            F2.start {
                              inner.chunks
                                ...
                                .interruptWhen(done.map(_.nonEmpty)) // must be AFTER enqueue to the sync queue, otherwise the process may hang to end last item while being interrupted
                                .compile
                                .drain
                                .attempt
                                .flatMap { r =>
