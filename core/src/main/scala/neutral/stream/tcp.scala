...
object tcp {
  ...
  def merge[A](a: Process[Connection,A], a2: Process[Connection,A])(implicit S:Strategy): Process[Connection,A] =
    wye(a,a2)(neutral.stream.wye.merge)
  ...
  /** Like [[neutral.stream.tcp.write]], but ignores the output. */
  ...
  /** Like [[neutral.stream.tcp.writes]], but ignores the output. */
  ...
  /** Like [[neutral.stream.tcp.lastWrites]], but ignores the output. */
