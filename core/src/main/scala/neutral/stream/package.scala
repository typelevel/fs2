...
import neutral.stream.Process.Env

package object stream {

  // Temporary aliases to get our arms around this task
  type \/[+A, +B]  = scala.Either[A, B]
  val  \/          = scala.Either
  type -\/[+A]     = scala.Left[A, Nothing]
  val  -\/         = scala.Left
  type \/-[+B]     = scala.Right[Nothing, B]
  val  \/-         = scala.Right
  def left[A](a: A) = scala.Left(a)
  def right[B](b: B) = scala.Right(b)
